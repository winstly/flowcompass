...
level: 2
pipeline: [pm-execution-planning, pm-plan-user-review, pm-fullchain-execution]
...
<Do_Not_Use_When>
- 用户已明确批准——直接进入 pm-fullchain-execution
- 计划还在草拟中——回到 pm-execution-planning
...
- 用户多次拒绝同一计划 → 可能需要回到 pm-execution-planning 重新制定
- 用户要求缩小范围 → 回到 pm-execution-planning 调整方案
...
```markdown
## [TIMESTAMP] pm-plan-user-review | {level} level
