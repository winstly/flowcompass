...
level: 3
pipeline: [dynamic-monitoring, internal-self-check, visual-verification]
handoff-policy: auto
...
<Do_Not_Use_When>
- 任务尚未执行完成（用 fullchain-execution）
- 需要专业QA质检（用 dual-qa-review）
- 只需要查看进度不需要检验（用 dynamic-monitoring）
</Do_Not_Use_When>
...
```markdown
## [TIMESTAMP] internal-self-check | {level} level
