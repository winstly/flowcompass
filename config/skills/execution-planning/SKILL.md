...
level: 2
pipeline: [requirements-consolidation, execution-planning, plan-user-review]
handoff-policy: auto
...
<Do_Not_Use_When>
- 需求尚未锁定（用 requirements-consolidation）
- 已在执行阶段（用 dynamic-monitoring）
- 只需要对计划审批（用 plan-user-review）
</Do_Not_Use_When>
...
- 资源不足或需求有歧义 → 回溯至 requirements-consolidation 或升级
- 规划暂缓 → 保留已规划内容，记录待决项
...
```markdown
## [TIMESTAMP] execution-planning | {level} level
