...
level: 1
pipeline: [requirements-initial-review, requirements-alignment, roster-management]
...
<Do_Not_Use_When>
- 需求概要还没做（回到 requirements-initial-review）
- 已经在执行中（太晚了，用 dynamic-monitoring）
- 正在做技术方案设计（用 execution-planning）
- 需要深度拆解需求细节（用 subagent-analysis）
</Do_Not_Use_When>
...
```markdown
## [TIMESTAMP] requirements-alignment | {level} level
