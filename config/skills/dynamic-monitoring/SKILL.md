...
level: 3
pipeline: [fullchain-execution, dynamic-monitoring, internal-self-check]
handoff-policy: auto
...
<Do_Not_Use_When>
- 尚未启动执行（用 fullchain-execution）
- 执行已完成需要自检（用 internal-self-check）
- 只需要查看进度不需要协调
</Do_Not_Use_When>
...
7. **记录 Agent 级别执行数据**
   - 对 team-board.md 中每个 Agent 记录以下指标：
     - `tasks_assigned`：分配的任务数
     - `tasks_completed`：已完成的任务数
     - `issues_found`：巡检发现的问题数（按 Critical/Major/Minor 分级）
     - `blocked_count`：被阻塞次数
     - `response_quality`：沟通响应质量（优/良/差）
   - 写入 `.knowledge/org/agent-performance.json`，格式：
     ```json
     {
       ...
       "agents": {
         "{codename}": {
           "tasks_assigned": 3,
           "tasks_completed": 2,
           "issues_found": {"critical": 0, "major": 1, "minor": 2},
           "blocked_count": 1,
           "response_quality": "良"
         }
       }
     ...
     ```
   - 该数据供 evaluation-system 评分使用，评分前必须有此数据
</Steps>
...
```markdown
## [TIMESTAMP] dynamic-monitoring | {level} level
