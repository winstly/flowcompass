...
level: 2
pipeline: [subagent-analysis, requirements-consolidation, execution-planning]
...
<Do_Not_Use_When>
- SubAgent分析尚未完成（用 subagent-analysis）
- 需求已锁定且无变更（用 execution-planning）
- 只需要单路结果不需要汇总
</Do_Not_Use_When>
...
5. **Wiki维护**
   - 创建 wiki/summaries/architecture/final.md
   - 创建 wiki/summaries/architecture/conflicts.md
   - 创建 wiki/summaries/architecture/glossary.md（DEEP级别）
...
<Tool_Usage>
- 写入工具：Write / Edit
- 产出路径：wiki/summaries/architecture/final.md, wiki/summaries/architecture/conflicts.md, wiki/summaries/architecture/glossary.md
</Tool_Usage>
...
- 记录：wiki/summaries/architecture/conflicts.md #C003
...
- 冲突无法解决 → 升级至决策者，记录分歧点
- 汇总暂缓 → 保留已汇总内容，标记未完成项
- SubAgent产出严重缺失 → 回退至 subagent-analysis
</Escalation_And_Stop_Conditions>
...
- [ ] 冲突是否全部解决并有记录？
- [ ] 终版需求是否有版本号和锁定日期？
- [ ] wiki/summaries/architecture/final.md 是否已写入？
- [ ] wiki/index.md 是否已更新？
...
```markdown
## [TIMESTAMP] requirements-consolidation | {level} level

**Type**: requirements_consolidation
**Level**: {QUICK|STANDARD|DEEP}
**Changes**:
- Created wiki/summaries/architecture/final.md
