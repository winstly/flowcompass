...
level: 1
pipeline: [project-closure-iteration, structural-decomposition, requirements-initial-review]
handoff-policy: auto
...
- 正在执行具体实现任务——用 execution-planning
- 只需要修改单个文件——直接动手，无需拆解
</Do_Not_Use_When>
...
所有 wiki 产出文件应包含以下 frontmatter 元数据，用于知识库保鲜：

```yaml
---
generated_at: 2025-03-20T10:30:00Z
source_files:
  - src/example.ts
skill_name: structural-decomposition
status: fresh | stale
---
```

- `generated_at`：生成时间戳
- `source_files`：生成此文档时参考的源文件列表
- `skill_name`：生成此文档的 skill 名称
- `status`：fresh（最新）或 stale（源文件已变更，需要刷新）
...
```markdown
## [TIMESTAMP] structural-decomposition | {level} level
