...
level: 2
pipeline: [roster-management, subagent-analysis, requirements-consolidation]
handoff-policy: auto
...
<Do_Not_Use_When>
- 主Agent尚未选型（用 roster-management）
- 分析已完成，需要汇总（用 requirements-consolidation）
- 任务简单到单一Agent即可完成
</Do_Not_Use_When>
...
5. **Wiki维护**
   - 创建 wiki/summaries/architecture/sub-agent-tasks.md
   - 创建 wiki/summaries/architecture/sub-agent-results/ 目录
...
- 写入工具：Write / Edit
- 产出路径：wiki/summaries/architecture/sub-agent-tasks.md, wiki/summaries/architecture/sub-agent-results/
</Tool_Usage>
...
- 分析暂缓 → 保留已收集结果，标记未完成项
- SubAgent间冲突严重 → 记录分歧点，交由 requirements-consolidation 仲裁
</Escalation_And_Stop_Conditions>
...
- [ ] 冲突是否显式标记而非掩盖？
- [ ] 独立任务是否并行启动？
- [ ] wiki/summaries/architecture/sub-agent-tasks.md 是否已写入？
- [ ] wiki/index.md 是否已更新？
...
```markdown
## [TIMESTAMP] subagent-analysis | {level} level

**Type**: subagent_analysis
**Level**: {QUICK|STANDARD|DEEP}
**Changes**:
- Created wiki/summaries/architecture/sub-agent-tasks.md
